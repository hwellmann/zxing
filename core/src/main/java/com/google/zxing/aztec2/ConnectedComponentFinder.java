...
/**
 * Finds connected components in a bit matrix. Each connected component is a
 * maximal set of pixels of the same colour such that any two pixels of the
 * component are connected by a path in the same component. A path is a sequence
 * of adjacent pixels. A pixel is defined to be adjacent with its four matrix
 * neighbours (north, east, south, west). Alternatively, if the {@code diagonal}
 * flag is set, the diagonal neighbours (north-west, north-east, south-east,
 * south-west) are also defined as adjacent, so that a pixel has up to eight
 * neighbours.
 * <p>
 * {@code findConnectedComponents()} must be called before calling
 * {@code getComponentMap()} or {@code getLabel()}.
 * <p>
...
public class ConnectedComponentFinder {
	...
	private static Logger log = LoggerFactory
			.getLogger(ConnectedComponentFinder.class.getSimpleName());
	...
	private int[] neighbourLabels = new int[8];
	...
	/**
	 * Constructs a connected component finder for the given bit matrix.
	 * 
	 * @param matrix
	 *            bit matrix
	 * @param diagonal
	 *            if true, diagonally adjacent pixels are considered as
	 *            connected
	 */
	public ConnectedComponentFinder(BitMatrix matrix, boolean diagonal) {
		this.matrix = matrix;
	...
	/**
	 * Expands the envelope of the component with the given label by including
	 * the given point and increments the pixel count.
	 * 
	 * @param x
	 * @param y
	 * @param label
	...
	private void expandComponent(int x, int y, int label) {
		...
		if (component == null) {
			...
			component = new ConnectedComponent(label, 0, envelope, matrix.get(
					x, y));
	...
	/**
	 * Labels the given pixel. If all neighbours are unlabelled, a new label is
	 * assigned to the given pixel. Otherwise, the pixel is labelled with the
	 * smallest neighbour label, and this label is marked as parent of all other
	 * neighbour labels.
	 * 
	 * @param x
	 *            row index
	...
	/**
	 * Checks a neighbour of a given pixel. If the neighbour and the given pixel
	 * have the same colour, the neighbour's label (if present) is added to the
	 * set of neighbour labels.
	 * 
	...
	/**
	 * Returns the label of the given pixel, if its colour matches the given
	 * one.
	 * 
	 * @param i
	 *            row index
	 ...
	 * @param bit
	 *            expected colour
	 * @return pixel label or null, if pixel is unlabelled or has the opposite
	 *         colour
	 */
	...
	/**
	 * Updates the label of the given pixel, traversing the parent hierarchy up
	 * to the root, replaces the label, if necessary. Also increments the pixel
	 * count for the final label.
	 * 
	 * @param x
	 *            row index
	 * @param y
